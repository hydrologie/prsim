...
### Lognormal
rLNORMLD = function(n, meanlog = 0, sdlog = 1, max_prob){
  cat(paste0(" LD LOG Normal max_prob=  ", max_prob, "\n"))
  if (max_prob == 0.9997) {
    max_prob = 0.9999
  } else if (max_prob == 0.9999) {
    max_prob = 0.9999
  }
  F <- runif(n, min = 1e-10, max = max_prob)
  x = qlnorm(F, meanlog =meanlog, sdlog = sdlog)
  #cat(paste0(" LD function F=  ", F, " ", "x= " ,x , " ", "n=", n, " ",  "mean= ", meanlog, "sd= ", sdlog, "\n"))
  return(x)
}

pLNORMLD <- function(x, theta)  plnorm(x, theta[1], theta[2])
LNORMLD_fit <- function( xdat, ...)   fitdistr( xdat, "log-normal", show=FALSE, ...)$estimate
...
rgumbelLD = function(n, mu= 0, sigma = 1, max_prob){
  #max_prob = 0.99999
  cat(paste0(" LD GUMMBEL function F=  ", max_prob, "\n")) 
  if (max_prob == 0.9997) {
    max_prob = 0.9999
  } else if (max_prob == 0.9999) {
    max_prob = 0.99999
  ...
  #cat(paste0(" LD GUMMBEL function F=  ", max_prob, "\n"))
  F <- runif(n, min = 1e-10, max = max_prob)
  x = qgumbel(F, mu, sigma)
  return(x)
}
rGUMBEL <- function(n, theta, max_prob=0.99999)  rgumbelLD(n, theta[1], theta[2], max_prob)
...
rGUMBELFIX = function(n, mu= 0, sigma = 1, max_prob){
  cat(paste0(" LD GUMMBELFIX function F=  ", max_prob, "\n"))
  max_prob = 0.9997+1-1
  F <- runif(n, min = 1e-10, max = max_prob)
  x = qgumbel(F, mu, sigma)
  return(x)
...
pGUMBELFIX <- function(x, theta)  pgumbel(x, theta[1], theta[2])
GUMBELFIX_fit <- function( xdat, ...)   fitdist(xdat, "gumbel",start=list(a=10, b=10))$estimate
...
# ### KAPPAFIX
# rand.kappa<- function(numerosita, xi, alfa, k, h)  rand.kappaFIX( numerosita, xi, alfa, k, h, max_prob)
# require(FAdist)
# pKAPPAFIX <- function(x, shape1=1,shape2=1,scale=1,location=0)  pkappa4(x, shape1=1,shape2=1,scale=1,location=0)
# dKAPPAFIX <- function(x, shape1=1,shape2=1,scale=1,location=0)  dkappa4(x, shape1=1,shape2=1,scale=1,location=0)
# KAPPAFIX_fit <- function(xdat, ...)   fitdist(xdat, "KAPPAFIX", start=list(shape1=1,shape2=1,scale=1,location=0))$estimate
# 
#  rKAPPAFIX <- function(numerosita, shape1, shape2,scale=1,location=0, max_pob) {
#     F <- runif(numerosita, min = 1e-10, max = max_prob)
#     x <- qkappa4(F,shape1,shape2,scale=1,location=0,lower.tail=TRUE,log.p=FALSE)
#     cat(paste0(" LD KAPPAFIX function F=  ", F, "\n"))
#     return(x)
#   }
...
prob_list<-rep(list(0.9997),times=365) ### must contain one probability value per day
...
prsim.wave.marg <- function(data, station_id="Qobs", number_sim=1, win_h_length=15, 
                            ...
                            GoFtest=NULL, verbose=TRUE, suppWarn=FALSE, prob_list=rep(list(0.9997),times=365),...){  
  ...
  for(l in 1:length(data)){
    ...
    if(marginal_list[[l]]!="kappa" & marginal_list[[l]]!="empirical"){
      ...
      for(d in c(1:365)){
        ...
        #cat(paste0("Marginal ",marginal_list[[l]],  " l=", l, " before rCDF:\n"))
        data_random <- rCDF[[l]](n=length(data_window), theta, max_prob=prob_list[[d]])
      ...
      for(d in c(1:365)){    
        ...
        if(marginal_list[[l]]=="kappa"){
          ...
          data_day$kappa <- rand.kappa(length(data_day$Qobs),
                                       ...
                                       k=par_day_list[[l]][d,"k"],h=par_day_list[[l]][d,"h"],max_prob=prob_list[[d]])
        ...
        if(marginal_list[[l]]!="kappa" & marginal_list[[l]]!="empirical"){
          ...
          data_day$cdf <-   rCDF[[l]](n=length(data_day$Qobs), par_day_list[[l]][d,],max_prob=prob_list[[d]])
...
filename<-paste(dir_main, "obs_outaouais_harm_bassins_sup.Rdata", sep='')
#filename<-paste(dir_main, "obs_outaouais_harm_arnpriorchang-01-2021.Rdata", sep='')
...
liste_des_distributions_choisies[1:51]<- 'kappa'
nbre_de_parametres_par_distribution[1:51]<- 4

# 
# #Cabonga
 liste_des_distributions_choisies[3]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[3]<- 2
# 
# # Carillon
 liste_des_distributions_choisies[4]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[4]<- 2
# 
# #Des Joachims
 liste_des_distributions_choisies[8]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[8]<- 2
# 
# #Dozois
 liste_des_distributions_choisies[9]<- 'GUMBELFIX'
 nbre_de_parametres_par_distribution[9]<- 2
# 
# #High Falls
 liste_des_distributions_choisies[10]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[10]<- 2
 
# #Kiamika
 liste_des_distributions_choisies[12]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[12]<- 2
 
# #Lac des Quinzes
 liste_des_distributions_choisies[14]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[14]<- 2

 
 # #Lac Victoria
 liste_des_distributions_choisies[17]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[17]<- 2
 
 # #Lady Evelyn
 liste_des_distributions_choisies[18]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[18]<- 2
 
 # #Madawaska
 liste_des_distributions_choisies[20]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[20]<- 2
 
 # #Michinamecus
 liste_des_distributions_choisies[24]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[24]<- 2 
 
 # #Otta Holden
 liste_des_distributions_choisies[27]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[27]<- 2 
 
 # #Rapide-7
 liste_des_distributions_choisies[31]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[31]<- 2 
 
 
 # #Coulonge
 liste_des_distributions_choisies[34]<- 'LNORMLD'
 nbre_de_parametres_par_distribution[34]<- 2 
 
 # #Dumoine
 liste_des_distributions_choisies[35]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[35]<- 2 
 
 # #Kinojevis
 liste_des_distributions_choisies[36]<- 'gumbelLD'
 nbre_de_parametres_par_distribution[36]<- 2 
 ...
 prob_list <- c(rep(list(0.9997),times=31),rep(list(0.9997),times=28),rep(list(0.9997),times=31), ### jan to march
                rep(list(0.9997),times=30),rep(list(0.9997),times=31),rep(list(0.9997),times=30), ### april to june
                rep(list(0.9999),times=31),rep(list(0.9999),times=31),rep(list(0.9999),times=30), ### july to september
                rep(list(0.9999),times=31),rep(list(0.9999),times=30),rep(list(0.9997),times=31)) ### october to september
...
for(i in start_sim_number:(start_sim_number)) {
  out <- prsim.wave.marg(data=runoff_multi_sites, number_sim=1, 
                        ...
                        n_par_list=nbre_de_parametres_par_distribution,
                        prob_list=prob_list)#probleme avec goftest ks_test ks.test
  ...
  # Trace les probabilite par saison
